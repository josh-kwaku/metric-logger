...
export class MetricService {
  ...
  private sum(lines: Array<string>, key: string): number {
    let sum = 0;
  ...
  //   /**
  //    * Updates metric file with only less than an hour old data
  //    * @param key
  //    * @param newFileContents
  //    */
  //   private updateMetricFile(key: string, content: string) {
  //     const writer = fileSystem.getWriteStream(pathToMetricFile + `${key}.txt`);
  //     writer.on("error", err => {
  //       let error = new AppError(
  //         err.message,
